...
const userController = {
  ...
  //     const currentUser = await Clockindata.findOne({ userEmail });

  //     // Get the current date
  //     let currentDate = new Date();

  //     // Check if the user has already clocked in today
  //     let lastStatusDate = currentUser.userStatus.length > 0 ? new Date(currentUser.userStatus[currentUser.userStatus.length - 1].clockInTime) : null;
  //     if (
  //       lastStatusDate &&
  //       currentUser.userStatus[currentUser.userStatus.length - 1].clockedIn &&
  //       lastStatusDate.getDate() == currentDate.getDate() &&
  //       lastStatusDate.getMonth() == currentDate.getMonth() &&
  //       lastStatusDate.getFullYear() == currentDate.getFullYear()
  //     ) {
  ...
  //     // If user is not within range, do not create an entry
  //     if (!isWithin) {
  //       return res.status(400).json({ message: "User is not within range" });
  //     }

  //     // Create a Date object from the org clock-in time string
  ...
  //     // Check if the current time is less than or equal to the organization's clock-in time
  //     let isOnTime = currentDate <= orgClockInTime;

  //     // Prepare status update object
  //     let statusUpdate = { clockedIn: true, onTime: isOnTime, clockInTime: currentDate };

  //     let message = isOnTime ? "User clocked in successfully on time" : "User clocked in late";

  //     // Update user status and add new status to the userStatus array
  //     await Clockindata.findOneAndUpdate(
  //       { email: userEmail },
  //       { $set: { companyId: companyId }, $push: { userStatus: statusUpdate } },
  //       { new: true }
  //     );
